...
const navItems = [
  { label: 'Dashboard', path: `${BASE_PATH}/hr/dashboard` },
  { label: 'Employees', path: `${BASE_PATH}/hr/employees` },
  { label: 'Documents', path: `${BASE_PATH}/hr/documents` },
  { label: 'Verification', path: `${BASE_PATH}/hr/verification` }
];
        ...
        <nav style={{ padding: '1rem' }}>
          {navItems.map((item) => (
            <NavLink
              key={item.label}
              to={item.path}
              end={item.path === `${BASE_PATH}/hr/dashboard`}
              style={({ isActive }) => ({
                display: 'block',
